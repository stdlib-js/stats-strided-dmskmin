...
var dmskmin = require( './../lib/ndarray.js' );
...
tape( 'the function has an arity of 7', function test( t ) {
	t.strictEqual( dmskmin.length, 7, 'has expected arity' );
	t.end();
});
...
tape( 'the function calculates the minimum value of a strided array according to a mask', function test( t ) {
	...
	v = dmskmin( x.length, x, 1, 0, mask, 1, 0 );
...
tape( 'if provided an `N` parameter less than or equal to `0`, the function returns `NaN`', function test( t ) {
	...
	v = dmskmin( 0, x, 1, 0, mask, 1, 0 );
	t.strictEqual( isnan( v ), true, 'returns expected value' );

	v = dmskmin( -1, x, 1, 0, mask, 1, 0 );
	t.strictEqual( isnan( v ), true, 'returns expected value' );
